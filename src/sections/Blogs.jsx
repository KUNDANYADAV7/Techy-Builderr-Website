...
//         <div className="grid grid-cols-1 sm:grid-cols-1 lg:grid-cols-1 xl:grid-cols-3 gap-12" data-aos="zoom-in" data-aos-delay="200">
//           {sortedBlogs.length > 0 ? (
//             sortedBlogs.slice(0, 6).map((blog, index) => (
//               <div className="group relative w-full rounded-lg overflow-hidden shadow-md transform hover:scale-105 transition-transform duration-300" key={index}>
  ...
  return (
    <div className="w-full bg-white py-10">
      <div className="w-[95%] m-auto bg-[#F3F5F5] py-5 px-6 md:px-20 rounded-xl shadow-lg">
        <section id="blogs" className="w-full h-fit rounded-xl">
          <div className="mx-auto my-10 p-2">
            <h1 data-aos="zoom-in" className="text-[32px] font-semibold leading-10 text-[#000045] mb-6">
              Recent Blogs
            </h1>
                ...
                sortedBlogs.slice(0, 6).map((blog, index) => (
                  <div className="group relative w-full rounded-lg overflow-hidden shadow-md transform hover:scale-105 transition-transform duration-300" key={index}>
                    <Link to={`/blog/${blog._id}`}>
                      ...
                      {/* Text Overlay (Bottom) */}
                      <div className="absolute bottom-4 left-4 text-white">
                        <h2 className="text-lg font-semibold group-hover:text-yellow-500">{blog?.title}</h2>
                        <p className="text-sm">{blog?.category}</p>
                      </div>
                    </Link>
                  </div>
